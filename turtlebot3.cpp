...
static const TB3ModelInfo burger_info = {
    "Burger", 1, 0.033, 0.160, 0.080, 0.105, false,
};

static const TB3ModelInfo waffle_info = {
    "Waffle", 2, 0.033, 0.287, 0.1435, 0.220, false,
};

static const TB3ModelInfo waffle_with_manipulator_info = {
    "Waffle_OpenManipulator", 3, 0.033, 0.287, 0.1435, 0.220, true,
};
...
// 속도 배열 수정
static float goal_velocity[VelocityType::TYPE_NUM_MAX] = {0.0, 0.0, 0.0};              // 리프트 속도 추가
static float goal_velocity_from_cmd[VelocityType::TYPE_NUM_MAX] = {0.0, 0.0, 0.0};     // 리프트 속도 추가
static float goal_velocity_from_rc100[VelocityType::TYPE_NUM_MAX] = {0.0, 0.0, 0.0};   // 리프트 속도 추가
static float goal_velocity_from_button[VelocityType::TYPE_NUM_MAX] = {0.0, 0.0, 0.0};  // 리프트 속도 추가
...
static void test_lift_motor(void); 
/*******************************************************************************
...
enum ControlTableItemAddr {
  ...
  ADDR_CMD_VEL_LIFT = 182,           // 리프트 속도 명령을 위한 새 주소
  ADDR_PRESENT_POSITION_LIFT = 144,  // 리프트 위치를 위한 새 주소
  ADDR_PRESENT_VELOCITY_LIFT = 136,  // 리프트 속도를 위한 새 주소
  ADDR_PRESENT_CURRENT_LIFT = 128,   // 리프트 전류를 위한 새 주소
  ADDR_PROFILE_ACC_LIFT = 186,       // 리프트 가속 프로필을 위한 새 주소
...
typedef struct ControlItemVariables {
  ...
  int32_t cmd_vel_lift;
  int32_t present_position_lift;
  int32_t present_velocity_lift;
  int16_t present_current_lift;
  uint32_t profile_acceleration_lift;
  // manipulator
...
void TurtleBot3Core::begin(const char* model_name) {
  ...
  dxl_slave.addControlItem(ADDR_PRESENT_POSITION_LIFT, control_items.present_position[MortorLocation::LIFT]);
  dxl_slave.addControlItem(ADDR_PRESENT_VELOCITY_LIFT, control_items.present_velocity[MortorLocation::LIFT]);
  dxl_slave.addControlItem(ADDR_PRESENT_CURRENT_LIFT, control_items.present_current[MortorLocation::LIFT]);
  dxl_slave.addControlItem(ADDR_CMD_VEL_LIFT, control_items.cmd_vel_lift);
  dxl_slave.addControlItem(ADDR_PROFILE_ACC_LIFT, control_items.profile_acceleration[MortorLocation::LIFT]);
  if (p_tb3_model_info->has_manipulator == true) {
...
void TurtleBot3Core::run() {
  ...
  if (millis() - pre_time_to_control_motor >= INTERVAL_MS_TO_CONTROL_MOTOR) {
    ...
    if (get_connection_state_with_motors() == true) {
      ...
      // 리프트 모터 별도 제어
      motor_driver.control_lift(goal_velocity[VelocityType::LIFT_MOTION]);
    }
  }
...
static void update_goal_velocity_from_3values(void) {
  ...
  goal_velocity[VelocityType::LIFT_MOTION] = goal_velocity_from_button[VelocityType::LIFT_MOTION] +
                                            goal_velocity_from_cmd[VelocityType::LIFT_MOTION] +
                                            goal_velocity_from_rc100[VelocityType::LIFT_MOTION];
...
void update_motor_status(uint32_t interval_ms)
{
  ...
  int16_t current_l, current_r, current_lift;
  ...
  if (millis() - pre_time >= interval_ms)
  {
    ...
    if (get_connection_state_with_motors() == true)
    {
      int32_t position_l, position_r, position_lift;
      int32_t velocity_l, velocity_r, velocity_lift;
      
      motor_driver.read_present_position(position_l, position_r, position_lift);
      control_items.present_position[MortorLocation::LEFT] = position_l;
      control_items.present_position[MortorLocation::RIGHT] = position_r;
      control_items.present_position[MortorLocation::LIFT] = position_lift;
      
      motor_driver.read_present_velocity(velocity_l, velocity_r, velocity_lift);
      control_items.present_velocity[MortorLocation::LEFT] = velocity_l;
      control_items.present_velocity[MortorLocation::RIGHT] = velocity_r;
      control_items.present_velocity[MortorLocation::LIFT] = velocity_lift;
      
      if (motor_driver.read_present_current(current_l, current_r, current_lift) == true)
      {
        control_items.present_current[MortorLocation::LEFT] = current_l;
        control_items.present_current[MortorLocation::RIGHT] = current_r;
        control_items.present_current[MortorLocation::LIFT] = current_lift;
      }
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t& dxl_err_code, void* arg) {
  ...
  switch (item_addr) {
    ...
    case ADDR_MOTOR_TORQUE:
      if (get_connection_state_with_motors() == true) motor_driver.set_torque(control_items.motor_torque_enable_state);
      break;
    ...
    case ADDR_CMD_VEL_LIFT:
      goal_velocity_from_cmd[VelocityType::LIFT_MOTION] = constrain((float)(control_items.cmd_vel_lift * 0.01f), -1.0, 1.0);  // 필요에 따라 범위 조정
      break;

    case ADDR_PROFILE_ACC_LIFT:
      if (get_connection_state_with_motors() == true) {
        uint32_t left_acc, right_acc;
        motor_driver.read_profile_acceleration(left_acc, right_acc);
        motor_driver.write_profile_acceleration(left_acc, right_acc, control_items.profile_acceleration[MortorLocation::LIFT]);
      }
      break;
...
static bool get_connection_state_with_ros2_node() { return connection_state_with_ros2_node; }

static void set_connection_state_with_ros2_node(bool is_connected) { connection_state_with_ros2_node = is_connected; }
...
static bool get_connection_state_with_motors() { return is_connected_motors; }

static void set_connection_state_with_motors(bool is_connected) { is_connected_motors = is_connected; }
...
static bool get_connection_state_with_joints() { return is_connected_joints; }

static void set_connection_state_with_joints(bool is_connected) { is_connected_joints = is_connected; }
...
void test_motors_with_buttons(uint8_t buttons) {
  static bool move[VelocityType::TYPE_NUM_MAX] = {false, false, false};  // 리프트를 위해 크기 3으로 확장
  static int32_t saved_tick[3] = {0, 0, 0};  // 리프트 틱 저장용 추가
  static double diff_encoder = 0.0;

  int32_t current_tick[3] = {0, 0, 0};  // 리프트 틱 저장용 추가

  if (get_connection_state_with_motors() == true) {
    // 리프트 포함 모든 위치 읽기
    motor_driver.read_present_position(current_tick[MortorLocation::LEFT], current_tick[MortorLocation::RIGHT], current_tick[MortorLocation::LIFT]);
  }

  if (buttons & (1 << 0)) {
    // 첫 번째 버튼 - 직진 및 리프트 상승
    move[VelocityType::LINEAR] = true;
    move[VelocityType::LIFT_MOTION] = true;  // 리프트 모션 활성화
    saved_tick[MortorLocation::RIGHT] = current_tick[MortorLocation::RIGHT];
    saved_tick[MortorLocation::LIFT] = current_tick[MortorLocation::LIFT];  // 리프트 위치 저장
  ...
  if (move[VelocityType::LINEAR]) {
    if (abs(saved_tick[MortorLocation::RIGHT] - current_tick[MortorLocation::RIGHT]) <= diff_encoder) {
      ...
      goal_velocity_from_button[VelocityType::LIFT_MOTION] = 0.3;  // 리프트 상승 (양수 값)
    } else {
      goal_velocity_from_button[VelocityType::LINEAR] = 0.0;
      goal_velocity_from_button[VelocityType::LIFT_MOTION] = 0.0;  // 리프트 정지
      move[VelocityType::LINEAR] = false;
      move[VelocityType::LIFT_MOTION] = false;  // 리프트 모션 비활성화
    }
  } else if (move[VelocityType::ANGULAR]) {
    if (abs(saved_tick[MortorLocation::RIGHT] - current_tick[MortorLocation::RIGHT]) <= diff_encoder) {
      goal_velocity_from_button[VelocityType::ANGULAR] = -0.7;
      goal_velocity_from_button[VelocityType::LIFT_MOTION] = -0.3;  // 리프트 하강 (음수 값)
    } else {
  ...
  } else if (move[VelocityType::LIFT_MOTION]) {
    // 리프트만 움직이는 경우 처리 (만약 다른 모션이 중지되어도 리프트는 계속 작동해야 할 경우)
    if (abs(saved_tick[MortorLocation::LIFT] - current_tick[MortorLocation::LIFT]) <= diff_encoder) {
      // 이미 다른 조건에서 속도가 설정되어 있으므로 여기서는 별도로 설정할 필요 없음
    } else {
      goal_velocity_from_button[VelocityType::LIFT_MOTION] = 0.0;  // 리프트 정지
      move[VelocityType::LIFT_MOTION] = false;  // 리프트 모션 비활성화
    }
...
void test_lift_motor() {
  // static uint32_t pre_time = 0;
  // static bool toggle = false;

  // if (millis() - pre_time >= 3000) {  // 3초마다
  //   pre_time = millis();
  //   toggle = !toggle;
    
  //   // 리프트 모터를 위아래로 움직이게 함
  //   float lift_speed = toggle ? 0.3 : -0.3;
    
  //   // 직접 제어 (즉시 모터 제어)
  //   motor_driver.control_lift(lift_speed);
    
  //   // 또는 속도 설정 (메인 루프에서 처리)
  //   goal_velocity_from_button[VelocityType::LIFT_MOTION] = lift_speed;
    
  //   DEBUG_PRINT("Test lift motor: ");
  //   DEBUG_PRINTLN(lift_speed);
  // }
}